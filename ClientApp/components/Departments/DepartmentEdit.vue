  ...
  <div class="col-10 offset-1">
    <form v-on:submit.prevent="editItem">
      <b-container fluid>
        ...
        <div
          v-if="errors.has('Факультет')"
          class="offset-3 alert-validate"
        >{{ errors.first('Факультет') }}</div>

        <b-row class="pad-4">
          <b-col cols="3" class="text-right">Декан:</b-col>
          <b-col cols="9">
            <b-form-input size="sm" name="decan" v-model="model.decan"></b-form-input>
          </b-col>
        </b-row>
        ...
        <div
          v-if="errors.has('Телефон2')"
          class="offset-3 alert-validate"
        >{{ errors.first('Телефон2') }}</div>
        ...
        <div class="col-10 offset-2">
          ...
          <department-specialities-selected-list
            @del-speciality="onDel"
            :list="this.model.specialities"
          ></department-specialities-selected-list>
...
import departmentSpecialitiesList from "./DepartmentSpecialitiesList";
import departmentSpecialitiesSelectedList from "./DepartmentSpecialitiesSelectedList";
// import XLSX from "xlsx";

export default {
  components: {
    vSelect,
    departmentSpecialitiesList,
    departmentSpecialitiesSelectedList
    // XLSX
  },
  ...
  mounted: function() {
    ...
    // ПЕРЕДЕЛАТЬ ВСЕ В КОНТРОЛЛЕРЕ Departments и как в PrepodEdit
    axios
      .get("api/Departments/" + this.$route.params.id)
      .then(function(response) {
        self.model = response.data;
        self.selectedFaculty.value = self.model.facultyId;
        //self.model.specialities = self.specialities;
        axios.get(`api/faculties/names`).then(response => {
          self.faculties = response.data;
          if (self.selectedFaculty.value != null) {
            self.selectedFaculty.text = self.faculties.find(
              x => x.value === self.selectedFaculty.value
            ).text;
          }
          axios
            .get("api/specialities/selected/" + self.$route.params.id)
            .then(response => {
              self.model.specialities = response.data;
            });
        });
      });
    // this.model.specialities = this.specialities;
  },
    ...
    editItem() {
      this.$validator.validate().then(valid => {
        if (valid) {
          ...
          axios
            .put("api/departments/" + this.model.id, this.model)
            .then(response => {
